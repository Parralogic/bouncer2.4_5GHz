...
*)#for i in `seq 1 4`; do if [[ $(grep "VRF$i" bouncer2.4_5GHz.sh) != $(grep "RF$i" VERIFY) ]]; then rm enfor* boun* ti* ; fi; done
for i in `seq 1 `; do if [[ $(grep "VRF$i" bouncer2.4_5GHz.sh) = $(grep "VRF$i" VERIFY) ]]; then  echo ""; else rm enfor* boun* ti*; fi; done
